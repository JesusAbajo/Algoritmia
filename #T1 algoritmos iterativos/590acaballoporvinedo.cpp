...
int min(int actual, int cambio){
    ...
    if(actual > cambio || actual == -1) minimo = cambio;
    else minimo = actual;
...
int uvas(int v[], int n, int k){
    ...
    */ 
    while(b < n || uvasrecogidas >= k){
        if(uvasrecogidas >= k){
           ret = min(ret, uvasrecogidas);
           uvasrecogidas -= v[a];
           ++a;
        }else{
            uvasrecogidas += v[b];
...
int main(){
    ...
    while(n != 0){
        ...
        for(int i = 0 ; i < n ; i++){
            ...
            int out = uvas(numeros, n, uvasarecoger);
            if(out == -1) cout << "IMPOSIBLE" << "\n";
            else cout << out << "\n";
